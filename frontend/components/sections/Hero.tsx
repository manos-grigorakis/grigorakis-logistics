...
export default function Hero() {
  ...
  return (
    <section className="relative flex items-center justify-around min-h-screen px-4 overflow-hidden">
      {/* Background */}
      <div className="absolute pointer-events-none inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-size-[24px_24px]" />
      ...
      <div className="flex flex-col items-center justify-center text-center ">
        ...
        <h1 className="max-w-2xl mt-2 mb-10 font-bold leading-tight text-7xl">
        ...
        <div className="flex items-center">
          {stats.map((item, index) => (
            <div key={item.header} className="flex items-center">
              {index !== 0 && (
                <div className="w-px h-8 mx-8 bg-foreground/30" />
              )}
              <div className="w-32 text-center capitalize">
                <span className="text-2xl font-bold">
                  {" "}
                  <CountUp end={item.header} duration={1.5} separator="." />
                  {item.symbol}
                </span>
                <p className="text-sm font-light text-foreground/70 whitespace-nowrap">
                  {item.value}
                </p>
              </div>
            </div>
